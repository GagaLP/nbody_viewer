...
Light::Light(LightType type, bool enabled, Vector3&& position, Vector3&& target, Color&& color)
    : type(static_cast<int>(type)), enabled(enabled), position(position), target(target), color(color) {}
...
void Light::updatePosition(raylib::Shader& shader, raylib::Vector3&& position) {
    shader.SetValue(positionLoc, &position, SHADER_UNIFORM_VEC3);
}
